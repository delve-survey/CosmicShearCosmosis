...
FILES = {'wide' : '/project/chihway/data/decade/metacal_gold_combined_20240209.hdf',
         'n_of_z' : '/project/chihway/dhayaa/DECADE/SOMPZ/Runs/20240408/n_of_z.npy',
...
with h5py.File('/project/chihway/dhayaa/DES_Catalogs/DESY3_metacal_v03-004.h5') as m:
    
    ra  = m['catalog/unsheared/ra'][:][mask]
    dec = m['catalog/unsheared/dec'][:][mask]
    
    del mask
        
print("LOADED CATALOG. SIZE : ", ra.size)
...
#          From table I in https://arxiv.org/pdf/2105.13543
...
for i in range(4): output[:, i + 1] = X[X[:, 0] == i, 2]
