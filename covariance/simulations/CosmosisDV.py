...
OUTFILE = "baseline_cosmogrid_20240817.fits"
...
FILES = {'n_of_z' : '/project/chihway/dhayaa/DECADE/SOMPZ/Runs/20240408/Summary/mean_nz_combined_Final.npy',
         'Cov' : '/home/dhayaa/DECADE/CosmicShearCosmosis/covariance/simulations/FinalCov.npy',
        }

#STEP 1: Open the placeholder file and put into it the Covariance and n(z) that we want
placeholder_path = '/home/dhayaa/DECADE/CosmicShearCosmosis/make_datavectors/20240806/placeholder.fits'
f = fits.open(placeholder_path)

covmat, header_covmat = fits.getdata(placeholder_path, 'COVMAT',    header=True)
source, header_source = fits.getdata(placeholder_path, 'nz_source', header=True)
xip,    header_xip    = fits.getdata(placeholder_path, 'xip', header=True)
xim,    header_xim    = fits.getdata(placeholder_path, 'xim', header=True)
...
base_header = fits.open(placeholder_path)
...
[DEFAULT]
2PT_FILE = baseline_cosmogrid_20240817.fits
...
[sigma8_rescale]
file = /home/dhayaa/DECADE/CosmicShearCosmosis/make_datavectors/20240806/sigma8_rescale.py
...
os.system('rm shear.ini shear_test.ini values.ini priors.ini')